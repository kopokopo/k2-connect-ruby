...
RSpec.describe(K2ConnectRuby::K2Entity::K2Subscribe) do
  ...
  describe "#webhook_subscribe" do
    let(:empty_event) { webhook_structure("", "till", false, 112233) }
    let(:wrong_event) { webhook_structure("event_type", "till", false, 112233) }

    # Correct webhooks
    let(:b2b) { webhook_structure("b2b_transaction_received", "till", false, 112233) }
    let(:bg) { webhook_structure("buygoods_transaction_received", "till", false, 112233) }
    ...
    let(:bg_reversed) { webhook_structure("buygoods_transaction_reversed", "till", false, 112233) }
    # Incorrect Webhooks
    let(:incorrect_b2b) { webhook_structure("b2b_transaction_received", "company", false, 112233) }
    let(:incorrect_b2b) { webhook_structure("buygoods_transaction_received", "company", false, 112233) }
    let(:incorrect_customer_created) { webhook_structure("customer_created", "till", false, 112233) }
    let(:incorrect_settlement) { webhook_structure("settlement_transfer_completed", "till", false, 112233) }
    let(:incorrect_bg_reversed) { webhook_structure("buygoods_transaction_reversed", "company", false, 112233) }
  ...
  describe "#query_webhook" do
    it "should query recent webhook subscription" do
      ...
      access_token = K2ConnectRuby::K2Entity::K2Token.new("client_id", "client_secret").request_token
      k2_subscribe = K2ConnectRuby::K2Entity::K2Subscribe.new(access_token)
      bg = webhook_structure("buygoods_transaction_received", "till", false, 112233)
  ...
  def webhook_structure(event_type, scope, enable_daraja_payload = false, scope_reference = nil)
    {
      event_type: event_type,
      url: Faker::Internet.url,
      scope: scope,
      scope_reference: scope_reference,
      enable_daraja_payload: enable_daraja_payload,
    }
  end
